...
@Service
public class IuserService implements IuserMapper{

    @Autowired
    private IuserMapper iuserMapper;
    public List<User> ListUser() {
        return iuserMapper.ListUser();
    }
    ...
    public boolean batchImport(String fileName, MultipartFile file) throws Exception {
        ...
        for (User userResord : userList) {
            ...
            List<User> userByName = iuserMapper.findUserByName(name);
            if (userByName.size()==0) {
                iuserMapper.insertUser(userResord);
                System.out.println(" 插入 "+userResord);
            } else {
                iuserMapper.Update(userResord);
